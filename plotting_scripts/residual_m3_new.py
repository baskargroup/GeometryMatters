...
class ResidualLoss(FEM2D):
    ...
    def forward(self, dataX, dataY, Y_true):
        ...
        utrue_gp = self.gauss_pt_evaluation(utrue)
        vtrue_gp = self.gauss_pt_evaluation(vtrue)
        ...
        ptrue_x_gp = self.gauss_pt_evaluation_der_x(ptrue)
        ptrue_y_gp = self.gauss_pt_evaluation_der_y(ptrue)
        
        r1_gp = (u_gp * u_x_gp + v_gp * u_y_gp + p_x_gp)**2
        r2_gp = (u_gp * v_x_gp + v_gp * v_y_gp + p_y_gp)**2

        error_x_gp = (u_x_gp - utrue_x_gp)**2 + (v_x_gp - vtrue_x_gp)**2
        error_y_gp = (u_y_gp - utrue_y_gp)**2 + (v_y_gp - vtrue_y_gp)**2

        r1_elm_gp = r1_gp * JxW
        r2_elm_gp = r2_gp * JxW
        error_x_elm_gp = error_x_gp * JxW
        error_y_elm_gp = error_y_gp * JxW
        ...
        error_x_total_squared = torch.sum(torch.sum(error_x_elm, -1), -1)
        error_y_total_squared = torch.sum(torch.sum(error_y_elm, -1), -1)
        deriv_error_Total = (error_x_total_squared + error_y_total_squared).pow(2).mean() / (self.domain_size**2)

        return error_x_elm, error_y_elm, r1_elm, r2_elm, deriv_error_Total
